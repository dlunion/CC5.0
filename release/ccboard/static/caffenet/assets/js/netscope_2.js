...
module.exports = AppController = (function() {
  ...
  AppController.prototype.showEditor = function(loader) {
    if(globalEditor == null){
	  globalEditor = this.netEditor = new Editor(this.makeLoader(loader.load));
	  return globalEditor;
    }
	  
    /**
    if (_.isUndefined(window.CodeMirror)) {
      return $.getScript('/static/caffenet/assets/js/lib/codemirror.min.js', (function(_this) {
        return function() {
...
layers.Uniform = this.UniformLayer = (function() {
  ...
  UniformLayer.prototype.inferShapes = function(bottoms, tops) {
    ...
    for (i = j = 0, ref = tops.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      ...
      if(tops != undefined && i < tops.length && bottoms != undefined && i < bottoms.length){
...
isLossLayer = function(layerType) {
  return /loss/i.test(layerType) || (/cpp/i.test(layerType));
};

isDataLayer = function(layerType) {
  return (/input/i.test(layerType)) || (/cpp/i.test(layerType)) || (/data/i.test(layerType));
};

isUniformLayer = function(lt) {
  return (/relu/i.test(lt)) || (/cpp/i.test(lt)) || (/prelu/i.test(lt)) || (/elu/i.test(lt)) || (/sigmoid/i.test(lt)) || (/tanh/i.test(lt)) || (/abs/i.test(lt)) || (/power/i.test(lt)) || (/exp/i.test(lt)) || (/log/i.test(lt)) || (/bnll/i.test(lt)) || (/threshold/i.test(lt)) || (/bias/i.test(lt)) || (/scale/i.test(lt)) || (/lrn/i.test(lt)) || (/dropout/i.test(lt)) || (/batchnorm/i.test(lt)) || (/mvn/i.test(lt)) || (/softmax/i.test(lt));
};
...
exports.inferTopShapes = function(node) {
  ...
  try {
    ...
    if(/cpp/i.test(node.type)){
      for(var i = 0; i < node.tops.length; ++i)
        node.tops[i].shape = [1, 3, 800, 800];

      for(var i = 0; i < node.bottoms.length; ++i)
        node.bottoms[i].shape = [1, 3, 800, 800];
    }
...
module.exports = Loader = (function() {
  ...
  Loader.prototype.fromGist = function(gistID, callback) {
	...
	globalShowEditor();
	globalEditor.editor.setValue("");

    var _this = this;
    echo("/gists/" + gistID)
    .succ(function(data){
    	globalEditor.editor.setValue(data.files.prototxt.content);
    	_this.load(data.files.prototxt.content, callback);
    })
    .err(function(err){
    	window.onerror(err.msg);
    })
    .post();
  };
...
$(document).ready(function() {
  ...
  globalShowEditor = function(){
	  app.showEditor(loader);
  }
