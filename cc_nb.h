...
#define Empty	DynamicValue()
	...
	struct OThreadContextSession{
		...
		virtual void put(const char* key, const void* value) = 0;
		...
		static void this_thread_put(const char* key, const void* value);

		template<typename _T>
		static _T& this_thread_get_(const char* key){
			return *((_T*)this_thread_get(key));
		}

		template<typename _T>
		static void this_thread_put_(const char* key, const _T& value){
			this_thread_put(key, &value);
		}

#define ThreadPut(name, var)			OThreadContextSession::this_thread_put_(name, var)
#define ThreadGet(name, type)			OThreadContextSession::this_thread_get_<type>(name)
	...
	namespace layers{
		...
		//    Axpy层定义
		//
		struct OAxpy : public OLayerOp{

			virtual string serial_param(){ return ""; }
			virtual const char* caffe_type_name(){ return "Axpy"; }
		};
		...
		Tensor conv2d(const Tensor& x, const vector<int>& kernel, const string& padding = "same",
			const vector<int>& strides = { 1, 1 }, const vector<int>& dilations = { 1, 1 }, const string& name = "", bool bias_term = true, int group = 1);
		Tensor deformableConv(const Tensor& x, const vector<int>& kernel, const vector<int>& padding = { 2, 2 },
			const vector<int>& strides = { 1, 1 }, const vector<int>& dilations = { 2, 2 }, const string& name = "", int group = 1);
		...
		vector<Tensor> split(const Tensor& x, const vector<int>& point, int axis = 1, const string& name = "");
	...
	namespace train{
		...
		namespace caffe{
			...
			//    训练任务执行
			//
			std::shared_ptr<cc::Solver> run(const Optimizer& optimizer, const std::function<StepEndCallbackFunctional>& stepEndCallback = nullptr, const std::function<PreTrainCallbackFunctional>& preTrainCallback = nullptr);
		};
	};
};
