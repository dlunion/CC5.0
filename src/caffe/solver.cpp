...
extern void onOptimizationStopped(cc::Solver* solver, bool early, int iters, float smoothed_loss);
extern void onTestClassification(cc::Solver* solver, float testloss, int index, const char* itemname, float itemscore);
...
template<typename Dtype>
void Solver<Dtype>::postEarlyStopSignal(){
	g_do_early_stop_signal = true;
}
  ...
  while (iter_ < stop_iter) {
	...
	if (Caffe::root_solver()){
		if (this->ccSolver_){
			...
			if (callback){
				void* userdata = this->ccSolver_->getStepEndCallbackUserData();
				callback(this->ccSolver_, iter_, smoothed_loss_, userdata);
			}
		}
	}
	...
	if (g_do_early_stop_signal){
		request = SolverAction::STOP;
	}

    // Save a snapshot if needed.
    if (((param_.snapshot() && iter_ % param_.snapshot() == 0) || (request == SolverAction::SNAPSHOT || g_do_snapshot_signal)) && Caffe::root_solver()) {
...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  if (requested_early_exit_) {
	  onOptimizationStopped(this->ccSolver(), true, iter_, smoothed_loss_);
  ...
  onOptimizationStopped(this->ccSolver(), false, iter_, smoothed_loss_);
}
...
void Solver<Dtype>::TestClassification(const int test_net_id) {
  ...
  for (int i = 0; i < test_score.size(); ++i) {
	  ...
	  if (loss_weight) {
		  classification_loss += loss_weight * mean_score;
	  }
  }
  ...
  for (int i = 0; i < test_score.size(); ++i) {
	...
	if (loss_weight){
		onTestClassification(this->ccSolver(), classification_loss, i, output_name.c_str(), loss_weight * mean_score);
	}
	else{
		onTestClassification(this->ccSolver(), classification_loss, i, output_name.c_str(), mean_score);
	}
  }
}
