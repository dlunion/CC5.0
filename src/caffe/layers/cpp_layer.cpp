...
extern cc::BaseLayer* customInstanceGetBaseLayer(cc::CustomLayerInstance instance);
...
void CPPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	if (ccNewLayerFunction){
		...
		if (this->ccLayer()){
			this->ccLayer()->setBaseLayerInstance(customInstanceGetBaseLayer(layerInstance_));
		}
	}
	else{
