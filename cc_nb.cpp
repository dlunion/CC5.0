...
namespace cc{
	...
	struct OThreadContextSessionImpl : public OThreadContextSession{
		...
		static std::list<std::function<StepEndCallbackFunctional>> step_end_callback_list_;
		static std::function<PreStepEndCallbackFunctional> pre_step_end_callback_;
		static std::function<PreStepEndCallbackFunctional> over_step_end_callback_;
		static std::list<std::function<PreTrainCallbackFunctional>> pre_train_callback_list_;
		static mutex callback_lock_;
		...
		map<std::string, const void*> value_store_map_;
		...
		static void callPreTrain(OThreadContextSession* session){
			if (pre_train_callback_list_.empty()) return;

			std::unique_lock<std::mutex> l(callback_lock_);

			auto iter = pre_train_callback_list_.begin();
			for (; iter != pre_train_callback_list_.end(); ++iter)
				(*iter)(session);
		}

		static void callStepEnd(OThreadContextSession* session, int step, float smoothed_loss){
			if (step_end_callback_list_.empty()) return;

			std::unique_lock<std::mutex> l(callback_lock_);

			if (pre_step_end_callback_) pre_step_end_callback_(session, step, smoothed_loss);
			auto iter = step_end_callback_list_.begin();
			for (; iter != step_end_callback_list_.end(); ++iter){
				(*iter)(session, step, smoothed_loss);
			}
			if (over_step_end_callback_) over_step_end_callback_(session, step, smoothed_loss);
		}
		...
		virtual void put(const char* key, const void* value){
	...
	std::function<PreStepEndCallbackFunctional> OThreadContextSessionImpl::pre_step_end_callback_;
	std::function<OverStepEndCallbackFunctional> OThreadContextSessionImpl::over_step_end_callback_;
	std::list<std::function<StepEndCallbackFunctional>> OThreadContextSessionImpl::step_end_callback_list_;
	std::list<std::function<PreTrainCallbackFunctional>> OThreadContextSessionImpl::pre_train_callback_list_;
	mutex OThreadContextSessionImpl::callback_lock_;
	...
	void OThreadContextSession::set_pre_step_end_callback(const std::function<PreStepEndCallbackFunctional>& callback){
		OThreadContextSessionImpl::pre_step_end_callback_ = callback;
	}

	void OThreadContextSession::set_over_step_end_callback(const std::function<OverStepEndCallbackFunctional>& callback){
		OThreadContextSessionImpl::over_step_end_callback_ = callback;
	}

	void OThreadContextSession::add_pre_train_callback(const std::function<PreTrainCallbackFunctional>& callback){

		if (callback){
			std::unique_lock<std::mutex> l(OThreadContextSessionImpl::callback_lock_);
			OThreadContextSessionImpl::pre_train_callback_list_.push_front(callback);
		}
	}

	void OThreadContextSession::add_step_end_callback(const std::function<StepEndCallbackFunctional>& callback){

		if (callback){
			std::unique_lock<std::mutex> l(OThreadContextSessionImpl::callback_lock_);
			OThreadContextSessionImpl::step_end_callback_list_.push_front(callback);
		}
	}
	...
	void OThreadContextSession::this_thread_put(const char* key, const void* value){
	...
	string OOptimizer::seril(){
		...
		if (!test_iter.empty()){
			for (size_t i = 0; i < test_iter.size(); ++i)
		...
		if (!one_epoch_iter_size.empty()) result += f("one_epoch_iter_size: %f\n", one_epoch_iter_size.floatval());
	...
	string OLayerOp::serial(){
		...
		for (size_t i = 0; i < loss_weight.size(); ++i)
		...
		bool hasBias = true;
		if (string(caffe_type_name()) == "Convolution"){
			layers::OConv2D* conv = static_cast<layers::OConv2D*>(this);
			hasBias = conv->bias_term;
		}
		else if (string(caffe_type_name()) == "InnerProduct"){
			layers::ODense* dense = static_cast<layers::ODense*>(this);
			hasBias = dense->bias_term;
		}

		if (bias_mult && hasBias)
	...
	namespace train{
		...
		namespace caffe{
			...
			void trainStepEndCallbackFunc(cc::Solver* solver, int step, float smoothed_loss, void* userData){
				...
				//}
				OThreadContextSessionImpl::callStepEnd(session, step, smoothed_loss);
			}
			...
			string readfromfile(const string& file){
				...
				if (len > 0){
					out.resize(len);
					if (fread((char*)out.data(), 1, len, f) != len)
						out.clear();
				}
				fclose(f);
				return out;
			...
			//    训练任务执行
			//
			std::shared_ptr<cc::Solver> run(const Optimizer& optimizer, const std::function<StepEndCallbackFunctional>& stepEndCallback, const std::function<PreTrainCallbackFunctional>& preTrainCallback){
				...
				if (optimizer->device_ids.empty()){
					...
					printf("train use GPU: root_device=%d, count=%d\n", optimizer->device_ids[0], (int)optimizer->device_ids.size());
				...
				if (stepEndCallback){
					OThreadContextSessionImpl::add_step_end_callback(stepEndCallback);
				}
				...
				//call pretrain
				OThreadContextSessionImpl::callPreTrain(session);

				if (!optimizer->device_ids.empty()){

					if (optimizer->device_ids.size() == 1)
						solver->installActionSignalOperator();
	...
	namespace layers{
		...
		string OSoftmax::serial_param(){
			...
			for (size_t i = 0; i < class_weight.size(); ++i)
		...
		Tensor conv2d(const Tensor&  x, const vector<int>& kernel, const string& padding,
			const vector<int>& strides, const vector<int>& dilations, const string& name, bool bias_term, int group){
		...
		Tensor deformableConv(const Tensor& x, const vector<int>& kernel, const vector<int>& padding,
			const vector<int>& strides, const vector<int>& dilations, const string& name, int group){
			...
			conv->kernel_initializer->type = "msra";
			conv->bias_initializer->type = "constant";
			conv->bias_initializer->value = 0;
			...
			layer->kernel_mult->decay_mult = 1;
			layer->kernel_mult->lr_mult = 1;
			...
			auto offset = conv2d(x, { 3, 3, conv->deformable_group * kernel[0] * kernel[1] * 2 }, "same", { 1, 1 }, { 2, 2 }, layer->name + "/offset");
			OConv2D* rconv = (OConv2D*)offset->owner.get();
			rconv->padding_size = {2, 2};
			//rconv->bias_term = false;

			//rconv->bias_initializer.reset(new cc::Initializer());
			//rconv->bias_initializer.reset();
			//rconv->bias_mult.reset();
			rconv->kernel_initializer.reset(new cc::Initializer());

			rconv->kernel_mult.reset(new ParamSpecMult());
			rconv->bias_mult.reset(new ParamSpecMult());
			rconv->kernel_mult->lr_mult = 0;
			rconv->kernel_mult->decay_mult = 1;
			rconv->bias_mult->lr_mult = 0;
			rconv->bias_mult->decay_mult = 2;

			rconv->kernel_initializer->type = "constant";
			rconv->kernel_initializer->value = 0.0;
		...
		vector<Tensor> split(const Tensor& x, const vector<int>& point, int axis, const string& name){
			...
			layer->output.resize(point.size() + 1);
			for (int i = 0; i < layer->output.size(); ++i){
				Tensor blob(new OTensor());
				blob->name = f("%s/%d", layer->name.c_str(), i);
				blob->owner = layer;
				layer->output[i] = blob;
			}
			return layer->output;
		...
		Tensor max_globalpooling2d(const Tensor& x, const string& name){
			return max_pooling2d(x, { 1, 1 }, { 1, 1 }, { 0, 0 }, true, name);
		}

		Tensor avg_globalpooling2d(const Tensor& x, const string& name){
			return avg_pooling2d(x, { 1, 1 }, { 1, 1 }, { 0, 0 }, true, name);
		}
		...
		Tensor product(const Tensor&  a, const Tensor&  b, const string& name){
			LayerOp layer(new OProduct());
			layer->name = layer->scope_name_or_next_auto_name(name);
			layer->output.resize(1);
			layer->input.resize(2);
		...
		Tensor axpy(const Tensor& scale, const Tensor& x, const Tensor& y, const string& name){

			OAxpy* axpylayer = new OAxpy();
			LayerOp layer(axpylayer);
			layer->name = layer->scope_name_or_next_auto_name(name);

			Tensor blob(new OTensor());
			blob->name = layer->name;
			blob->owner = layer;
			layer->input = { scale, x, y };
			layer->output = { blob };
			return blob;
		}

		Tensor scale(const Tensor& x, const Tensor& scale_data, const string& name){
